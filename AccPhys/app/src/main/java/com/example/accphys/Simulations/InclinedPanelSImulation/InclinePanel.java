...
public class InclinePanel extends LinearLayout {

    private final EditText massInput;
    private final EditText frictionInput;
    private final EditText angleInput;
    ...
    public InclinePanel(Context context) {
        this(context, null);
    }

    public InclinePanel(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        angleInput = createStyledInput(context, "Angle (°)");

        inputRow.addView(massInput);
        inputRow.addView(frictionInput);
        inputRow.addView(angleInput);

        addView(inputRow, new LayoutParams(LayoutParams.WRAP_CONTENT, 0, 1f));
    ...
    public float getAngleInput() {
        try {
            return Float.parseFloat(angleInput.getText().toString());
        } catch (NumberFormatException e) {
            return 0;
        }
    }
